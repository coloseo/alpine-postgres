...
for path in $dockerfiles; do
  ...
  docker push "orangesys/alpine-postgres:${tag}"
  docker tag "orangesys/alpine-postgres:${tag}" "asia.gcr.io/orange-sys/alpine-postgres:${tag}"
  sudo /opt/google-cloud-sdk/bin/gcloud docker push asia.gcr.io/orange-sys/alpine-postgres:${tag}
done
